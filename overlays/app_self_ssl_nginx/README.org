...
Now you need to create a certificate based on the [[https://kubernetes.io/docs/concepts/configuration/secret/#tls-secrets][docs here]]. In our
sample manifest, we haven't commited it. You need to fix that before
deploying it. Make sure that don't wrap lines if you are using the
*base64* executable to do the encoding. Something like this will
properly produce the required content:
